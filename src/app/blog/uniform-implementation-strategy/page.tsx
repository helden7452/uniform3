import React from 'react';
import { ArticleHero } from '@/components/ArticleHero';
import { Container } from '@/components/Container';
import { ArticleBody } from '@/components/ArticleBody';
import { ArticleBreadcrumb } from '@/components/ArticleBreadcrumb';
import { AuthorCard } from '@/components/AuthorCard';
import { getAllAuthors } from '@/utils/getAllAuthors';
import Image from 'next/image';
import { MDXProvider } from '@/components/MDXProvider';

export const metadata = {
  title: 'Uniform Implementation Strategy',
  description: 'Learn about effective uniform implementation strategies for your organization',
};

export default async function Article() {
  const authors = await getAllAuthors();
  const teamAuthor = authors.find((author) => author.name === 'فريق فخر الخليج');

  return (
    <>
      <ArticleHero
        title="Uniform Implementation Strategy"
        image={{
          src: '/images/general-uniforms/uniform-implementation-strategy.jpg',
          alt: 'Uniform Implementation Strategy',
          width: 1024,
          height: 576,
        }}
      />

      <Container className="mt-8 sm:mt-10 lg:mt-12">
        <ArticleBreadcrumb
          breadcrumbs={[
            { label: 'الرئيسية', href: '/' },
            { label: 'المدونة', href: '/blog' },
            { label: 'Uniform Implementation Strategy', href: '/blog/uniform-implementation-strategy', isCurrent: true }
          ]}
        />
      </Container>

      <div className="bg-white py-8 sm:py-10">
        <Container>
          <div className="flex flex-col lg:flex-row gap-12">
            <ArticleBody className="lg:w-2/3">
              <MDXProvider>
                <div className="prose prose-slate max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-lead:text-slate-500 prose-a:font-semibold prose-a:underline">
                  <h2 className="text-xl font-bold mb-6">Uniform Implementation Strategy</h2>
                  
                  <p className="text-lg leading-relaxed mb-6">
                    Contenido del blog aquí...
                  </p>
                </div>
              </MDXProvider>
            </ArticleBody>

            <aside className="lg:w-1/3 space-y-8">
              {teamAuthor && (
                <div className="bg-white p-6 rounded-xl shadow-sm">
                  <h3 className="text-lg font-bold mb-4">كاتب المقال</h3>
                  <AuthorCard author={teamAuthor} />
                </div>
              )}
              
              
              <div className="bg-primary-50 p-6 rounded-xl shadow-sm">
                <h3 className="text-lg font-bold mb-4">خدمات ذات صلة</h3>
                <ul className="space-y-3">
                  <li>
                    <a href="/general-uniforms" className="flex items-center text-primary hover:text-primary-dark">
                      <span className="ml-2">⟨</span>
                      <span>/general-uniforms</span>
                    </a>
                  </li>
                </ul>
              </div>
            </aside>
          </div>
        </Container>
      </div>
    </>
  );
}
